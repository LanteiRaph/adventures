<template>
  <div class="container">
      ...
      <div class="card" v-for="(activity) in planned" :key="activity.id" data-text="Test">
        ...
        <div class="content">
          <div>
            ...
            <router-link :to="{name:'Single Detail', params:{current:activity.id, entity:ename}}" class="route">
              <span>Read More</span>
            </router-link>
          </div>
        </div>
    ...
    <div class="right_panel">
      ...
      <div class="bottom">
        <h2>Top Rated Adventure</h2>
        <div class="">
          <div class="card"  data-text="Test">
        <div class="imgbox">
          <img :src="require(`../assets/planned/${topRated.img}`)" />
        </div>
        <div class="content">
          <div>
            <h3>{{topRated.name}}</h3>
            <p>{{topRated.description}}</p>
            <button v-on:click="route(top.id)" class="route">
              <span>Read More</span>
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    route (valueId) {
      //
      // Save any needed data to the state for retriveing
      this.$store.state.single_details.ename = this.ename
      this.$store.state.single_details.current = valueId
      // Route to the single details componet.
      this.$router.push({ path: '/single_detail' })
    },
